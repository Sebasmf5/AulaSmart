    ...
    public static void invMixColumns(byte[][] state) {
        ...
        for (int col = 0; col < 4; col++) {
            for (int row = 0; row < 4; row++) {
                temp[row][col] = (byte) (
                                gmul(INV_MIX_COLUMNS_MATRIX[row][0], state[0][col]) ^
                                gmul(INV_MIX_COLUMNS_MATRIX[row][1], state[1][col]) ^
                                gmul(INV_MIX_COLUMNS_MATRIX[row][2], state[2][col]) ^
                                gmul(INV_MIX_COLUMNS_MATRIX[row][3], state[3][col])
                );
            }
        }
        // copiar de vuelta al estado original
        for (int row = 0; row < 4; row++) {
